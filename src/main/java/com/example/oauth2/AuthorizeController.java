...
public class AuthorizeController {
    ...
    public Object authorize(Model model, HttpServletRequest request) throws URISyntaxException, OAuthSystemException {
        ...
        try {
            ...
            System.out.println("clientId:"+authRequest.getClientId()+"--client:"+clientService.findByClientId(authRequest.getClientId()));
            if(!oAuthService.checkClientId(authRequest.getClientId())){
            ...
            if(!subject.isAuthenticated()){
                if(!login(subject, request)) {//登录失败时跳转到登陆页面
                    model.addAttribute("client",
                            clientService.findByClientId(authRequest.getClientId()));
                    model.addAttribute("redirectURI",authRequest.getRedirectURI());
                    return "oauth2login";
                }
            }
            ...
            OAuthASResponse.OAuthAuthorizationResponseBuilder builder = new OAuthASResponse.OAuthAuthorizationResponseBuilder(request, HttpServletResponse.SC_FOUND);
